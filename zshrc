...
function gw1 {
  ssh -t alex_stark@gw1.silver.musta.ch;
}

function gw2 {
  ssh -t alex_stark@gw2.silver.musta.ch;
}
...
function mpr {
  ...
  open "https://git.musta.ch/airbnb/$repo/compare/$from_branch?expand=1";
}

function dpb-kyoo-test {
  from_branch=$(git rev-parse --abbrev-ref HEAD)
  open "https://deployboard.d.musta.ch/apps/kyoo/snapshots/branches/$from_branch"
}

function dpb {
  open "https://deployboard.d.musta.ch/apps/$1/snapshots/"
}

function push {
  echo "Terminal command has compelted" | mailx -s $1 n7j5o1z1a5e5f4w4@airbnb.slack.com
}

export PATH="/usr/local/opt/mysql@5.6/bin:$PATH"

function usc {
  FORCE_BUILD=false ./update_sparse_checkout
}
...
function workon {
  python $HOME/stark-scripts/workon.py $@;
}

export MIR_NS_PREFIX=stark-development-cluster 
